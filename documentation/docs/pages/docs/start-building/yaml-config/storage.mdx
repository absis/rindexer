...
:::info
rindexer supports AWS RDS IAM authentication for enhanced security with automatic token rotation.
:::

If you're deploying to AWS and using RDS PostgreSQL, you can enable IAM authentication instead of using traditional passwords. This provides:

- ✅ Short-lived authentication tokens (15-minute TTL)
- ✅ Automatic token refresh every 5 minutes
- ✅ No long-lived database passwords in environment variables
- ✅ AWS CloudTrail audit logging for database access

#### Option 1: Enable via YAML Configuration

You can enable RDS IAM authentication directly in your `rindexer.yaml` file:
...
Alternatively, you can enable it using an environment variable (useful for overriding YAML config or when not using a YAML file):
...
:::info
**Configuration Precedence:** If both the YAML `rds_iam_auth` and the environment variable `USE_RDS_IAM_AUTH` are set, the environment variable takes precedence. This allows for runtime overrides without modifying your YAML configuration.
:::

:::warning
When using RDS IAM authentication, do not set `DATABASE_URL`. The connection will be configured automatically from the individual environment variables above.
:::

#### AWS Setup Requirements

To use RDS IAM authentication, you need to configure both AWS IAM and your RDS instance:

**1. Enable IAM Authentication on RDS:**
- Go to your RDS instance in AWS Console
- Modify the database and enable "IAM database authentication"
- Apply the changes (requires a brief restart)

**2. Create IAM Policy:**

Create an IAM policy that allows `rds-db:connect`:
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:rds-db:REGION:ACCOUNT_ID:dbuser:RESOURCE_ID/DATABASE_USER"
    }
  ]
}
```
...
- `RESOURCE_ID`: Your RDS resource ID (found in RDS console)
- `DATABASE_USER`: Your database username

**3. Attach Policy to IAM Role:**
- For EKS/ECS: Attach to your pod/task IAM role
- For EC2: Attach to your EC2 instance role
- For local development: Use AWS credentials with this policy
...
For EKS deployments with IAM roles for service accounts:
...
spec:
  template:
    spec:
      serviceAccountName: rindexer-sa  # With IAM role annotation
      containers:
      - name: rindexer
